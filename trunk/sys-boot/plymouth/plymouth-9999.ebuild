...
#	epatch "${FILESDIR}"/${PV}-gentoo-fb.patch
	## Adjust the FB for Gentoo
	sed 's/"\/dev\/fb"/"\/dev\/fb0"/g' src/plugins/renderers/frame-buffer/plugin.c > plugin_gentoo.c || die
	mv plugin_gentoo.c src/plugins/renderers/frame-buffer/plugin.c || die

	if use branding ; then
		cp ${FILESDIR}/gentoo_ply.png images/bizcom.png
        fi 
...
src_install() {
	...
	newinitd "${FILESDIR}"/plymouth.d plymouth
