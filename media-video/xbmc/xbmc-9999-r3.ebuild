...
use vdpau && ESVN_REPO_URI="https://xbmc.svn.sourceforge.net/svnroot/xbmc/branches/xbmc-vdpau/XBMC"
use pvr && ESVN_REPO_URI="https://xbmc.svn.sourceforge.net/svnroot/xbmc/branches/unified-pvr/XBMC"
DESCRIPTION="XBMC is a free and open source media-player and entertainment hub"
HOMEPAGE="http://xbmc.org/"
LICENSE="GPL-2"
SLOT="0"
KEYWORDS=""
IUSE="alsa autostart +debug external-libraries gprof joystick opengl pulseaudio pvr vdpau"
RDEPEND="opengl? ( virtual/opengl )
	external-libraries? ( media-video/ffmpeg )
	app-arch/bzip2
	app-arch/unrar
	app-arch/unzip
	app-arch/zip
...
pkg_setup() {
	if use pvr && use vdpau; then
		die "'pvr' and 'vdpau' track different subversion branches, and so cannot be enabled concurrently.  Please choose only one."
	fi

	if use pvr; then
		ewarn "Enabling the 'pvr' flag tracks an alternative branch, you"
		ewarn "probably shouldn't do this unless you know what you are doing"
		epause 3
	fi

	if use vdpau; then
		ewarn "Enabling the 'vdpau' flag tracks an alternative branch, you"
		ewarn "probably shouldn't do this unless you know what you are doing"
		epause 3
	fi

	# Video playback gives either segfaults or floating point exceptions #
	# if media-libs/alsa-lib is not built with 'debug' USE flag enabled #
	if ! built_with_use media-libs/alsa-lib debug; then
		die "media-libs/alsa-lib needs to be built with the 'debug' USE flag enabled."
	fi

	# media-libs/libsdl USE flag checks #
	...
	for flag in {alsa,X}; do
		if ! built_with_use media-libs/libsdl ${flag}; then
			die "media-libs/libsdl needs to be built with the '"${flag}"' USE flag enabled."
		fi
	done

	# media-libs/sdl-image USE flag checks #
	for flag in {gif,jpeg,png}; do
		if ! built_with_use media-libs/sdl-image ${flag}; then
...
src_unpack() {
	...
	epatch "${FILESDIR}/${P}_readsector.patch"
	...
	# Clean up XBMC's wrapper script #
		...
		use alsa && echo 'export SDL_AUDIODRIVER=alsa' >> tools/Linux/xbmc.sh.in
